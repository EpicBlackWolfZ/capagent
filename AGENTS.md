...
5. **Zero External Runtime Dependencies**: Compiles with `CGO_ENABLED=0` to a single statically linked binary without third-party runtime dependencies. Standard library is prioritized for core logic. The compile-time dependency `golang.org/x/sys/unix` is permitted in `internal/platform` for kernel-confined filesystem operations (`openat2(2)`); this dependency is permitted solely to enforce the M1.1 filesystem-security boundary, mechanically enforced by the import-boundary contract tests.
