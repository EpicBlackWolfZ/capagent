...
5. **Zero External Runtime Dependencies**: Compiles with `CGO_ENABLED=0` to a single statically linked binary without third-party runtime dependencies. Standard library is prioritized for core logic. The compile-time dependency `golang.org/x/sys/unix` is permitted in `internal/platform` for kernel-confined filesystem operations and read-only Linux host queries (`openat2(2)`); this dependency is permitted solely for the M1.1 filesystem-security boundary and M2 read-only host queries, mechanically enforced by the import-boundary contract tests.
...
7. **Filesystem Containment is Kernel-Backed**: All untrusted subpath access passes through `internal/platform.ScopedReader`. Each file operation is confined by the kernel via `openat2(2)` with `RESOLVE_IN_ROOT | RESOLVE_NO_MAGICLINKS`; lexical `ValidateSubpath` is a complementary first-layer check, never the only one. Per-op FD ownership is anchored to `readSubpath`; callbacks perform fd-relative I/O via direct `unix.*` syscalls and never close the FD themselves, eliminating double-close. FD 0 is a valid kernel-returned descriptor and is treated symmetrically with all other non-sentinel FD values; the closed-state sentinel is `-1`, not `0`. CLI binaries (`cmd/`) use `os.Args`, `os.Stdout`, `os.Stderr`, and `os.Exit` directly because those primitives are not in any denylist; file-I/O and `os/exec` are forbidden in `cmd/` exactly as in any other non-platform package.
8. **Command Authority is Explicit**: `CommandRunner.Run` accepts a `CommandSpec` with an absolute executable, literal arguments, an immutable `EnvPolicy`, and explicit directory/timeout semantics. Defaults are `PATH=/usr/bin:/bin`, `LC_ALL=C`, and working directory `/`; environment inheritance requires named capture through `NewEnvPolicy`. Credentials and namespaces remain current-process attributes. Raw output and explicitly inspected/unwrapped command data require sanitization before diagnostics or fixture capture.

### 2.1 Kernel Support

capagent requires **Linux 5.6 or newer** for `openat2(2)` with `RESOLVE_IN_ROOT` and `RESOLVE_NO_MAGICLINKS`. Older kernels return `ErrSymlinkUnsupported` from `NewScopedOSReader`; the caller is expected to fail fast.
...
- The platform third-party import allowlist contains exactly `golang.org/x/sys/unix`, not arbitrary external packages or its subpackages. Low-level syscall selectors outside platform are checked by the same scanner used for fixtures; harmless errno/constants remain permitted.
- `internal/app` is reserved for the application owner in #61. Lower layers MUST NOT depend on it. CLI internal imports are restricted to `internal/app` and `internal/version`; neither CLI nor application code receives a host-I/O exemption.
