...
The `inspection-*` scenarios cover successful collection, info failure, version failure that skips info, truncation, a missing requested field, version conflict an absent helper, and version/info drain failures. They evaluate `all(runtime.podman, runtime.podman.info)`. Helper metadata has its own observation and evidence references, so it cannot erase valid engine inspection. The info consumer key checks both predicates with evidence and a satisfied requirement:
...
`context-user-unavailable` replays a completed nonzero user-manager query (`accessible: false`); `context-user-drain` retains unknown accessibility after incomplete draining, even with a nonzero child exit. Both remain host/context collection reports without a deployment verdict.

## Integrated prerequisite fixtures
