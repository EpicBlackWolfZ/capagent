...
The foundation, fixture evaluation, host facts and execution-context milestones are closed. Baseline correctness fixes, OCI/helper observations, Quadlet prerequisites, engine configuration, storage and network/DNS assessment are delivered. The remaining delivery priorities are:

| Order | Outcome |
|---|---|
| 1 | Existing JSON requirements over live Podman evidence, with useful explanations. |
| 2 | Registry/image-policy configuration and remaining Podman lifecycle metadata. |
| 3 | [Qualified Podman assessment](https://github.com/EpicBlackWolfZ/capagent/issues/115), focused on rootless Quadlet/user systemd with rootful coverage. |
| 4 | Ansible integration and Docker/containerd baselines, then broader qualification and stable v1.0. |
