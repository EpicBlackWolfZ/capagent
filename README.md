...
The CLI collects [passive host facts](docs/host-facts.md) with `--json` and evaluates offline evidence with `--fixture DIR --json --pretty`. Local `--runtime podman` assessment adds target identity, executable discovery, [OCI/helper and Quadlet prerequisites](docs/podman-prerequisites.md), [engine configuration provenance](docs/podman-engine-config.md), [storage prerequisites](docs/podman-storage-config.md), and [network/DNS prerequisites](docs/podman-network-config.md). Explicit `--runtime podman --active` collects the selected CLI version and local effective runtime information, enabling qualified configuration source selection. It permits Podman startup writes; exit 0 establishes successful inspection, not workload readiness. Use [live assessment](docs/podman-assessment.md) with `--requirement FILE --explain` for a chosen prerequisite requirement and evidence explanations. See [Podman discovery](docs/podman-discovery.md) for command authority and [fixture evaluation](docs/fixture-evaluation.md) for offline outcomes and exits.
...
The foundation, fixture evaluation, host facts and execution-context milestones are closed. Baseline correctness fixes, OCI/helper observations, Quadlet prerequisites, engine configuration, storage, network/DNS assessment, live JSON requirements and evidence explanations are delivered. The remaining delivery priorities are:

| Order | Outcome |
|---|---|
| 1 | Registry/image-policy configuration and remaining Podman lifecycle metadata. |
| 2 | [Qualified Podman assessment](https://github.com/EpicBlackWolfZ/capagent/issues/115), focused on rootless Quadlet/user systemd with rootful coverage. |
| 3 | Ansible integration and Docker/containerd baselines, then broader qualification and stable v1.0. |
