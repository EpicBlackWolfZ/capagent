...
func TestProcfsReader_CustomRoot(t *testing.T) {
	...
	r := platform.NewProcfsReader(mem, "/testdata/proc")
	got, err := r.Filesystems()
	if err != nil {
		t.Fatalf("Filesystems: %v", err)
	}
...
func TestProcfsReader_DefaultRootIsProc(t *testing.T) {
	...
	r := platform.NewProcfsReader(mem, "")
	if r.Root() != "/proc" {
...
func TestProcfsReader_RealProcfs(t *testing.T) {
	...
	r := platform.NewProcfsReader(platform.NewOSPlatformReader(), "/proc")
...
func TestProcfsReader_RootOrDotSubpath(t *testing.T) {
	...
	mem.AddFile("/proc", []byte("Linux 6.0.0"), 0o644)

	r := platform.NewProcfsReader(mem, "/proc")
	for _, sub := range []string{".", "/", ""} {
		got, err := r.ReadProcFile(sub)
		if err != nil {
			t.Fatalf("ReadProcFile(%q): %v", sub, err)
		}
		if string(got) != "Linux 6.0.0" {
			t.Errorf("ReadProcFile(%q) = %q", sub, string(got))
		}
	}
}

// TestProcfsReader_ReadSelfEmpty exercises the ReadSelf short-circuit
// when subpath is empty.
func TestProcfsReader_ReadSelfEmpty(t *testing.T) {
	t.Parallel()

	mem := platform.NewMemPlatformReader()
	mem.AddFile("/proc/self/status", []byte("x"), 0o644)

	r := platform.NewProcfsReader(mem, "/proc")
	got, err := r.ReadSelf("status")
	if err != nil {
		t.Fatalf("ReadSelf: %v", err)
	}
	if string(got) != "x" {
